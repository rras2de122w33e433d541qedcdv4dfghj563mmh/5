...
local text = (os.date("[ %c ]=>  "..logtxt.."n n"))
...
function mutes_list(chat_id)
        local hash =  'mute:'..chat_id
        local list = redis:smembers(hash)
        local text = "Mutes for: [ID: "..chat_id.." ]:nn"
...
function muted_user_list(chat_id)
        local hash =  'mute_user:'..chat_id
        local list = redis:smembers(hash)
        local text = "Muted Users for: [ID: "..chat_id.." ]:nn"
        for k,v in pairsByKeys(list) do
                local user_info = redis:hgetall('user:'..v)
                if user_info and user_info.print_name then
                        local print_name = string.gsub(user_info.print_name, "_", " ")
...
function ban_by_reply(extra, success, result)
        ...
        if is_momod2(result.from.peer_id, result.to.peer_id) then -- Ignore mods,owner,admin
                ...
                send_large_msg(chat, "User @"..result.from.username.."["..result.from.peer_id.."] Banned")
         send_large_msg(channel, "User @"..result.from.username.."["..result.from.peer_id.."] Banned")
        else
                return
        end
end
...
function unban_by_reply(extra, success, result)
        ...
        if is_admin2(result.from.peer_id) then -- Ignore admins
                ...
                send_large_msg(chat, "User @"..result.from.username.."["..result.from.peer_id.."] unBanned")
                send_large_msg(channel, "User @"..result.from.username.."["..result.from.peer_id.."] unBanned")
